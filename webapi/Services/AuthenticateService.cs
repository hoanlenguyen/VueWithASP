                ...
                [FromBody] UserLoginDto input) =>
            {
                ...
                if (await userManager.CheckPasswordAsync(user, input.Password))
                {
                    var query = from s in db.UserRoles.AsNoTracking()
                                join sa in db.RoleClaims.AsNoTracking()
                                on s.RoleId equals sa.RoleId
                    ...
                    var claims = new List<Claim>
                    {
                        ...
                        new Claim(ClaimTypes.Name, user.UserDetail?.DisplayName ?? string.Empty),
                    ...
                    return Results.Ok(new
                    {
                        accessToken,
                        user.Email,
                        name = user.UserDetail?.DisplayName ?? string.Empty,
                        roleName,
                        permissions
                    });
                }
